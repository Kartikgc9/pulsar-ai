 ...
 * Extracted from Framer - Creates animated cursor trail effects
 * Variants: line, dots, particles, pixel
 */
...
interface MouseTrailProps {
  ...
  pixelSize?: number;
  snapToGrid?: boolean;
  blendMode?: 'source-over' | 'lighter' | 'screen';
  autoFade?: boolean;
  fadeDuration?: number;
...
function parseColor(col: string): RGB {
  if (col.startsWith('#')) {
    let hex = col.slice(1);
    ...
    (dt: number) => {
      ...
      if (variant === 'dots') {
        ...
        for (let i = 0; i < points.length; i++) {
          ...
          const size = pixelSize * (fadeOut ? 0.6 + 0.4 * a : 1);
          ctx.fillStyle = rgba(a, t);
